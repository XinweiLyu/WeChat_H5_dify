...
export default {
  ...
  methods: {
	  async getUserListFn(){
		  try {
			  ...
			  if (error.response && error.response.status === 401) {
				  ...
				  uni.showModal({
					  title: '需要重新授权',
					  content: '您的登录信息已过期，需要重新授权才能查看历史记录',
					  confirmText: '重新授权',
					  cancelText: '稍后再说',
					  success: (res) => {
						  if (res.confirm) {
    ...
    async prepareWechatAuth() {
      try {
        ...
        if (window.location.search.includes('code=')) {
          window.location.replace(url); // 刷新为无code参数的纯净URL
          return; // 让页面重新加载
        }
    ...
    viewHistory() {
      ...
      if (!token) {
        console.log('没有token，需要先授权');
        uni.showModal({
          title: '需要授权',
          content: '请先完成微信授权后再查看历史记录',
          confirmText: '立即授权',
          cancelText: '取消',
          success: (res) => {
            if (res.confirm) {
