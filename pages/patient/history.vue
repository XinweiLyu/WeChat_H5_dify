<template>
  ...
  />
  <!-- <div class="big_box" v-for="(item, index) in 100" :key="index">{{item}}</div> -->
  <view class="main">
  ...
  computed: {
    ...
    iframeSrc() {
      //const baseUrl = 'https://udify.app/chatbot/W6KLjKIF1kLPBQ8H';
	    const baseUrl = 'https://zbot.i-llusion.cn/chatbot/3NYcMUN1UMZ96FSG';
  ...
  handleTime(time) {
    ...
    handleXml(row) {
      ...
      getUserXml({ report_id: reportId }).then(res => {
        ...
        // 修复：正确访问响应数据结构中的xml字段
        const xmlString = res.data?.xml || res.xml || '';
